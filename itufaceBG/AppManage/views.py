...
def del_app_list(request):
    ...
    sql_path=MysqlHandle.get_xml_sql(xml_path='select_sql', xml_tag='select', xml_id='delete_file_path')#查找app路径的sql
...
def add_app(request):
    ...
    Rootpath = './static/appfile/{timestrap}/'.format(timestrap=str(int(time.time())))
    myFilemessage = ''
    ...
    if request.method == 'POST':
        ...
        try:
            sql = sql.format(appVersion=app_version, appType=appType, customerOruser=customerOruser, describe=describe,
                             appPath=appPath[9:], environment=environment,isFinal=isfinal)
            MysqlHandle.reConnect()
...
def update_app(request):
    ...
    if request.method=='POST':
        ...
        tag=MysqlHandle.delete_update_insert_mysql_data(sql.format(appType=appType,environment=environment,customerOruser=customeroruser,
        isFinal=isfinal,appVersion=app_version,describe=describe,id=ids))
        if tag==0 or tag==1:
            MysqlHandle.reConnect()
            data = MysqlHandle.select_mysql_data(sql_select.format(id=ids))[0]
            message='修改成功！'
